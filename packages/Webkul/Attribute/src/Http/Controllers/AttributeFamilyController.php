...
class AttributeFamilyController extends Controller
{
    ...
    public function store()
    {
        ...
        session()->flash('success', trans('admin::app.response.create-success', ['name' => '属性ファミリー']));
    ...
    public function update($id)
    {
        ...
        session()->flash('success', trans('admin::app.response.update-success', ['name' => '属性ファミリー']));
    ...
    public function destroy($id)
    {
        ...
        if ($this->attributeFamilyRepository->count() == 1) {
            session()->flash('error', trans('admin::app.response.last-delete-error', ['name' => '属性ファミリー']));

        } elseif ($attributeFamily->products()->count()) {
            session()->flash('error', trans('admin::app.response.attribute-product-error', ['name' => '属性ファミリー']));
        } else {
            try {
                $this->attributeFamilyRepository->delete($id);

                session()->flash('success', trans('admin::app.response.delete-success', ['name' => '属性ファミリー']));

                return response()->json(['message' => true], 200);
            } catch (\Exception $e) {
                report($e);
                session()->flash('error', trans('admin::app.response.delete-failed', ['name' => '属性ファミリー']));
            }
        }
    ...
    public function massDestroy()
    {
        ...
        if (request()->isMethod('delete')) {
            ...
            if (! $suppressFlash) {
                ...
                session()->flash('info', trans('admin::app.datagrid.mass-ops.partial-action', ['resource' => '属性ファミリー']));
            }
